...
SQL_ROW *db_mysql_run(MYSQL *mysql, const char* statement){
    ...
    while((mysqlrow = mysql_fetch_row(res))){
        ...
        if (!(row)){
            ...
            row->next = (SQL_ROW*)calloc(1, sizeof(SQL_ROW));
            if(!(row->next))
                goto end;
            row = row->next;
        }
...
int db_mysql_fmt(MYSQL *mysql, DB_OP op, char *buf, size_t bufsz, char *fmt, ...){
    int ret = -1;
...
SQL_ROW * db_mysql_select(MYSQL *mysql, char *buf, size_t bufsz, char *fmt, ...){
    va_list ap;
